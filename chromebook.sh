#!/bin/bash
mkdir -p ~/.xkb/{keymap,symbols,types,rules}
...
cp ./.xkb/symbols/mac_term ~/.xkb/symbols/mac_term
...
sleep 1
line=$(cat ~/.xkb/keymap/kbd.chromebook.term | grep -n 'xkb_types' | cut -f1 -d:)
sed -ie "${line}s/\"/)+mac_term(addmac_levels)\"/2" ~/.xkb/keymap/kbd.chromebook.term
sleep 1
line=$(cat ~/.xkb/keymap/kbd.chromebook.term | grep -n 'xkb_symbols' | cut -f1 -d:)
sed -ie "${line}s/\"/+chromebook(swap_lalt_lctrl)+mac_term(mac_levelssym)\"/2" ~/.xkb/keymap/kbd.chromebook.term
